...
.PHONY: run
run: $(IMAGE_NAME).iso
	qemu-system-x86_64.exe -M q35,smm=off -m 4G -cdrom $(IMAGE_NAME).iso -boot d -d int --no-reboot --no-shutdown -D qemu_int.log -serial stdio
...
run-uefi: ovmf $(IMAGE_NAME).iso
	qemu-system-x86_64.exe -M q35 -m 2G -bios ovmf/OVMF.fd -cdrom $(IMAGE_NAME).iso -boot d

.PHONY: run-hdd
run-hdd: $(IMAGE_NAME).hdd
	qemu-system-x86_64.exe -M q35 -m 2G -hda $(IMAGE_NAME).hdd

.PHONY: run-hdd-uefi
run-hdd-uefi: ovmf $(IMAGE_NAME).hdd
	qemu-system-x86_64.exe -M q35 -m 2G -bios ovmf/OVMF.fd -hda $(IMAGE_NAME).hdd
